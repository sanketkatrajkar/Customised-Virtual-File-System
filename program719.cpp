...
{ 
    ...
    printf("Current inodes reamening %d\n",superobj.FreeInodes);
   ...
   uareaobj.UFDT[i] ->  ptrinode -> FileSize = MAXFILESIZE;   // Max quota
   uareaobj.UFDT[i] ->  ptrinode -> ActualFileSize = 0;       // Empty initially
  ...
  //Decremenet the number of free iNodes 
   superobj.FreeInodes--;
...
)
{
    ...
    //file is not opned or created with the given fd -1
    if(uareaobj.UFDT[fd] == NULL)
    {
        return ERR_FILE_NOT_EXISTS;
    }
